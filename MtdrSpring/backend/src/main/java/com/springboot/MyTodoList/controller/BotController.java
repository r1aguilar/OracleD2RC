...
public class BotController extends TelegramLongPollingBot {
	...
	private static String sanitize(String raw) {
		...
		for (String ln : raw.split("\\R")) {
			...
			if (t.toLowerCase().startsWith("instrucción:"))
				continue;
	...
	private static boolean isValidStrict(String txt) {
		...
		if (!okText.matcher(nombre).matches())
			return false;

		if (!okText.matcher(descripcion).matches())
			return false;

		if (!prioridadRaw.matches("[123]"))
			return false;

		if (!fechaRaw.matches("(\\d{4}-\\d{2}-\\d{2}|0000-00-00)"))
			return false;
	...
	public BotController(String botToken, String botName, TareaService tareaService, UsuarioService usuarioService,
			ProyectoService proyectoService, SprintsService sprintsService,
			IntegrantesEquipoService integrantesEquipoService, EquipoService equipoService) {
			...
			try {
				...
				if (usuario.getBody() != null) {
					...
					BotHelper.sendMessageToTelegram(chatId,
							"No encontramos un usuario registrado con ese número. Contacta con soporte.", this);
			...
			if (chatState.startsWith("WAITING_FOR_UPDATE_TASK_")) {
				...
				if (tarea != null) {
					...
					try {
						...
						switch (field) {
							...
							case "Due date":
								...
								if (fechaIngresada.matches("\\d{4}-\\d{2}-\\d{2}")) {
									...
									fechaIngresada += "T23:59:59Z"; // Agregar la hora para que OffsetDateTime.parse no
																	// falle
						...
						BotHelper.sendMessageToTelegram(chatId, "Field " + field + " has been updated succesfully!",
								this);
						...
						if (field.equals("Due date")) {
							...
							BotHelper.sendMessageToTelegram(chatId,
									"Verify that the format is valid and the due date is in the range of the sprint dates.",
									this);
				...
				if (proyecto != null) {
					...
					BotHelper.sendMessageToTelegram(chatId, "El campo " + field + " ha sido actualizado correctamente.",
							this);
				...
				switch (messageTextFromTelegram) {
					...
					case "1":
						...
						BotHelper.sendMessageToTelegram(chatId, "Changed " + tarea.getNombre() + " status to Pending",
								this);
					...
					case "2":
						...
						BotHelper.sendMessageToTelegram(chatId, "Changed " + tarea.getNombre() + " status to Doing",
								this);
					...
					case "3":
						...
						BotHelper.sendMessageToTelegram(chatId,
								"Please input, in a whole number, the amount of real hours it took to complete the task",
								this);
				...
				if (parts.length == 4) {
					...
					try {
						...
						BotHelper.sendMessageToTelegram(chatId,
								"Task could not be created correctly. Priority must be a number between 1 and 3.",
								this);
					...
					if (sprintId != 0) {
						...
						idProyecto = equipoService.getItemById(integrantesEquipoService
								.getItemByIdUsuario(
										usuarioService.getItemByTelegramId(userTelegramId).getBody().getID())
								.getBody().getIdEquipo()).getBody().getIdProyecto();
					...
					if (response != null) {
						...
						BotHelper.sendMessageToTelegram(chatId,
								"Task could not be created correctly. Please try again.", this);
					...
					BotHelper.sendMessageToTelegram(chatId,
							"Task could not be created correctly. Please use the correct format:\n\nName\nDescription\nPriority (From 1 to 3)\nDue Date (YYYY-MM-DD)",
							this);
				...
				|| messageTextFromTelegram.equals(BotCommands.START.getCommand())) {
	...
	private void viewKPISDeveloperMenu(long chatId) {
		...
		try {
			...
			if (userInChat != null) {
				...
				ResponseEntity<IntegrantesEquipo> integranteResponse = integrantesEquipoService
						.getItemByIdUsuario(userInChat.getID());
				...
				if (integrante != null) {
					...
					if (equipo != null) {
						...
						// Despues de obtener la lista de sprints se hace el menu donde el callback sera
						// el id de cada sprint
	...
	private void showUpdateTaskStatusDeveloperMenu(long chatId) {
		...
		try {
			...
			// Primero buscas el equipo al que el user pertenece con el IntegranteEquipo by
			// idUsuario y

			// despues buscas en el repo de equipo por getItemById y de ese sacas el id del
			// proyecto para sacar despues los sprints
			...
			String[][] fields = {
					...
					{ "Due date",
							tarea.getFechaVencimiento() != null
									? String.valueOf(tarea.getFechaVencimiento()).substring(0, 10)
									: "No Due Date" },

					{ "Story Points",
							tarea.getStoryPoints() != null ? String.valueOf(tarea.getStoryPoints())
									: "No Story Points" },

					{ "Estimated Time", tarea.getTiempoEstimado() != null ? String.valueOf(tarea.getTiempoEstimado())
							: "No Estimated Time" }
	...
	private void handleCallbackQuery(Update update) {
		...
		if (callbackData.startsWith("ACCEPT_TASK")) {
			...
			BotHelper.sendMessageToTelegram(chatId,
					"Please write the information of the task you are about to create,make sure it contains a Name, Description, Priority (From 1 being the least to 3 being the max), Due date",
					this);
			...
			if (parts.length == 3) {
				...
				field = parts[0].replace("MANAGER_UPDATE_TASK_", "") + " " + parts[1]; // Concatenar las dos primeras
																						// partes
	...
	private void showManagerListTeamTasks(long chatId, int idColumna, int idSprint, int idProyecto) {
		...
		try {
			...
			// Aqui tengo que listar todas las tareas ya sea pending doing o done
			// dependiendo del sprint y la columna
	...
	public void showManagerDeveloperTasks(long chatId, int idColumna, int idSprint, int idEncargado) {
		...
		try {
			...
			int idProy = equipoService
					.getItemById(integrantesEquipoService.getItemByIdUsuario(idEncargado).getBody().getIdEquipo())
					.getBody().getIdProyecto();
			...
			messageToTelegram.setText("Listing all " + typeTask + " tasks for "
					+ usuarioService.getItemById(idEncargado).getBody().getNombre() + " in " + scope);
	...
	private void showKPIsReportManagerFiltered(long chatId, int sprintId, int proyectId) {
		...
		try {
			...
			// Despues de obtener la lista de tareas, obtener la suma de las horas estimadas
			// como horas estimadas de trabajo, junto a las horas de trabajo reales
			...
			for (Tarea tarea : tareas) {
				...
				if (null != tarea.getIdColumna())
					switch (tarea.getIdColumna()) {
			...
			for (Usuario user : usuariosDelEquipo) {
				...
				tasksUserDoneTitleButton
						.setCallbackData("LIST_MANAGER_DEVELOPER_KPIS 3 " + sprintId + " " + userIdLocal);

				tasksUserDoingTitleButton
						.setCallbackData("LIST_MANAGER_DEVELOPER_KPIS 2 " + sprintId + " " + userIdLocal);

				tasksUserPendingTitleButton
						.setCallbackData("LIST_MANAGER_DEVELOPER_KPIS 1 " + sprintId + " " + userIdLocal);
	...
	private void showKPIsManagerSelectMenu(long chatId, int idProyecto) {
		...
		// Aqui se van a mostrar todos los sprints del proyecto y un boton para ver los
		// KPIs generales
	...
	private void showTeamMembersToManagerEditTask(long chatId, int taskId) {
		...
		try {
			...
			if (tareaResponse.getBody() != null) {
				...
				// Ahora que ya se tiene la lista de usuarios se crean los botones con cada
				// nombre del integrante y su id despues para cambiar en la tarea
	...
	private void showManagerEditTaskMenu(long chatId, int taskId) {
		...
		try {
			...
			String[][] fields = {
					...
					{ "Estimated Time",
							tarea.getTiempoEstimado() != null ? String.valueOf(tarea.getTiempoEstimado())
									: "No Estimated Time" },

					{ "Responsible",
							tarea.getIdEncargado() != null
									? usuarioService.getItemById(tarea.getIdEncargado()).getBody().getNombre()
									: "Not assigned" }
